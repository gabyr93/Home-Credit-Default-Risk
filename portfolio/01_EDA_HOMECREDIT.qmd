...
Most missing values are concentrated in apartment and household related variables (COMMONAREA, NONLIVINGAPARTMENTS, LIVINGAPARTMENTS), each with 68–70% missing, making them unreliable.

### Cleaning Data

1. **Drop** variables with >50% missing; **keep** those with ≤50%.  
...
- **Minimal missingness:** `CNT_FAM_MEMBERS`, `AMT_ANNUITY`, `AMT_GOODS_PRICE`, `DAYS_LAST_PHONE_CHANGE` — few gaps; simple imputation.  

- **Moderate missingness:** `EXT_SOURCE_2`, `OBS/DEF_*_SOCIAL_CIRCLE` — manageable; imputation feasible.  

- **Large gaps:** `EXT_SOURCE_1`, `EXT_SOURCE_3`, `TOTALAREA_MODE` — important predictors; retain with careful imputation.  

- **Credit bureau request variables:** (`AMT_REQ_CREDIT_BUREAU_*`) — 41,000 missing values likely mean no credit history; encode as zero or separate category.  
...
Overall, there’s a clear split: some variables only have a small number of extreme values, while others are heavily skewed with large outlier counts. The smaller cases won’t have much impact, but the variables with massive outliers will need closer attention. For modeling, applying transformations; e.g., log-scale or handling those extremes as a separate group will help prevent them from biasing the results.
...
```
The highest value in the AMT_INCOME column is 117,000,000, which is most likely a mistake. The median income is 147,150, which makes a lot more sense. Since that incorrect maximum was already removed, it doesn’t show up in the remaining outliers. After reviewing the rest of the income outliers, the values seem reasonable, so there’s no need to remove them.
...
- DAYS_EMPLOYED: Over 59,000 cases flagged, driven by the placeholder value 365243, meaning “no employment”. This should be recoded as missing.

- REGION_RATING_CLIENT / W_CITY: Around 78k–80k outliers; distribution is highly skewed. These may require grouping into broader categories instead of leaving as raw values.

- Credit Bureau Requests: Ranges from 43k–50k extreme cases, suggesting reporting spikes. These should be binned or capped to prevent extreme counts from dominating.
...
- **Credit Bureau Scores:** EXT_SOURCE_1, EXT_SOURCE_2, and EXT_SOURCE_3—which are strong predictors, were imputed with 0 to indicate no credit history. This prevents them from being dropped while preserving their interpretive value.
...
**🔟 Recommended Predictive Columns for Default Risk (TARGET)**
...
- EXT_SOURCE_1 – External credit score (from third-party bureau).

- EXT_SOURCE_2 – Strongest single predictor; higher score → lower default risk.

- EXT_SOURCE_3 – Another external score highly correlated with repayment ability.

- DAYS_BIRTH – Applicant’s age (negative values; older clients default less often).

- DAYS_EMPLOYED – Length of employment (longer stability → lower risk, though watch for outliers like 365243 “not employed”).

- AMT_CREDIT – Total credit amount requested. Large amounts relative to income raise risk.

- AMT_ANNUITY – Loan annuity; repayment burden predictor.

- AMT_GOODS_PRICE – Goods purchased with credit; important in relation to income.

- NAME_CONTRACT_TYPE – Cash vs revolving loans (revolving loans have higher default risk).

- REGION_RATING_CLIENT / REGION_RATING_CLIENT_W_CITY – Proxy for region-level economic risk (higher rating = lower default risk).
...
**✅ Your Predictors in the EDA Report**
...
Here’s the overlap between your predictors and my recommended set:
